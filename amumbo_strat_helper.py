...
def check_signals(sp500_data, vix_data, vix_thresh, sma_period):
    ...
    vix_data = calculate_sma(vix_data, 14)
    vix_data = calculate_sma(vix_data, 30)

    # Latest VIX value and short period SMA
    latest_vix = vix_data['Close'].iloc[-1].tolist() #[0]
    latest_vix_sma14 = vix_data['SMA14'].iloc[-1].tolist()
    latest_vix_sma30 = vix_data['SMA30'].iloc[-1].tolist()
    ...
    # Conditions
    high_volatility = (latest_vix > vix_thresh) or \
        (latest_vix_sma14 > vix_thresh*0.9) or (latest_vix_sma30 > vix_thresh*0.75) 
...
def plot_scraped_data(sp500_data, vix_data, vix_thresh, sma_period):
    ...
    ax1.fill_between(sp500_data.index, np.zeros_like(sp500_data['Close'])+min(sp500_data['Close']), sp500_data['Close'],
    ...
    count = 0
    for sma in vix_data.columns: #.get_level_values('Price'):
        if 'SMA' in sma: 
            ax2.plot(vix_data.index, vix_data[sma], label=f'VIX {sma}', color=VIX_SMA_COLORS[count], linestyle='--')
            count += 1
    ax2.axhline(y=vix_thresh, color='black', linestyle='solid', label='VIX Threshold')
    ...
    # Title and grid
    plt.title('S&P 500 Price, SMA200 and VIX Index Over the Last 2 Years')
    plt.grid(True)
    plt.show()
